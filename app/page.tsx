...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-sand to-cream">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-20 grid lg:grid-cols-2 gap-10 items-center">
          <div className="order-2 lg:order-1">
            <UmbrellaMotif className="w-full max-w-md mx-auto" />
          </div>
          <div className="order-1 lg:order-2">
            {fm.heroKicker && (
              <p className="section-label">{fm.heroKicker}</p>
            )}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-forest-900 leading-[1.12] mb-5">
              {fm.heroHeadline ?? "מכיבוי שריפות — למערכת חיים"}
            </h1>
      ...
      {/* ─── Why / Root cause ─── */}
      {home && (
        <ScrollReveal as="section" className="max-w-prose mx-auto px-4 sm:px-6 py-20">
          <p className="section-label">{fm.whyTitle ?? "למה עד היום לא הצלחתם לבד?"}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-forest-900 leading-tight">
              {fm.servicesTitle ?? "איך נוכל לעזור לכם?"}
            </h2>
          </div>
        ...
        <section className="max-w-5xl mx-auto px-4 sm:px-6 py-20">
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            {team.map((m) => (
              ...
              >
                {m.photo ? (
                  <span className="relative w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0 bg-sage-100">
                    <Image
                      src={m.photo}
                      alt={m.name}
                      fill
                  ...
                  <span className="w-16 h-16 rounded-2xl bg-sage-100 text-sage-700 flex items-center justify-center text-2xl font-extrabold flex-shrink-0">
                    {m.name.charAt(0)}
                  </span>
                )}
                <div>
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 py-20">
        <div className="relative overflow-hidden bg-sage-600 rounded-[28px] p-10 sm:p-14 text-center">
          ...
          <p className="text-white/80 mb-8 leading-relaxed max-w-lg mx-auto">
            שיחה קצרה, בלי התחייבות. נבין מה מטריד אתכם ונראה יחד איך אפשר לעזור.
          </p>
